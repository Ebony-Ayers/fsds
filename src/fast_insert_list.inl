...
constexpr void FastInsertList<T>::remove(size_t pos)
{
	...
	else
	{
		...
		else // there is a previous element
		{
			...
			if(element->next.nextData <= pos) // (ee^e^p# => ee^e^-#)
			{
				...
				//if element is right before pos form a chain. If it's not element's nextData will remain the same and thus doesn't need changing
				if(static_cast<size_t>(element - this->m_rawList.data()) + 1 == pos)
				{
	...
	: m_index(l.m_nextAvailable->next.nextData), m_nextAvailable(l.m_nextAvailable->next.nextAvailable)
	{
