...
{
	template<typename T>
	constexpr size_t InlineListDataBlock<T>::calculateRuntimeSizeOf(size_t predictedSize)
	{
		return sizeof(InlineListDataBlock) + (sizeof(T) * (predictedSize - 1));
	}
	template<typename T>
	...
	template<typename T>
	constexpr size_t InlineListDataBlock<T>::calculateNumElementsFromSize(size_t runtimeSize)
	{
		return (runtimeSize - sizeof(InlineListDataBlock) + sizeof(T)) / sizeof(T);
	}


	template<typename T>
	constexpr InlineListDataBlock<T>::InlineListDataBlock()
	: m_header(0, 0, 0)
	{ }
	template<typename T>
	constexpr InlineListDataBlock<T>::InlineListDataBlock(const size_t& size, const size_t& capacity, const size_t& front)
	: m_header(size, capacity, front)
	{ }
	...
	{
		fsds::listInternalFunctions::append(this->m_header, this->m_header, this->m_data, this->m_data, value);
	}
	template<typename T>
	constexpr void InlineListDataBlock<T>::copyAppend(const T& value, InlineListDataBlock<T>* newList)
	{
		fsds::listInternalFunctions::append(this->m_header, *newList->getHeader(), this->m_data, newList->getData(), value);
	}
	template<typename T>
	...
	{
		fsds::listInternalFunctions::prepend(this->m_header, *newList->getHeader(), this->m_data, newList->getData(), value);
	}
	template<typename T>
	...
	{
		fsds::listInternalFunctions::insert(this->m_header, *newList->getHeader(), this->m_data, newList->getData(), pos, value);
	}
	...
	template<typename T>
	constexpr void InlineListDataBlock<T>::removeWithoutDeconstruct(size_t pos)
	{
		fsds::listInternalFunctions::removeWithoutDeconstruct(this->m_header, this->m_data, pos);
	}
	template<typename T>
	constexpr void InlineListDataBlock<T>::removeBackWithoutDeconstruct()
	{
		fsds::listInternalFunctions::removeBackWithoutDeconstruct(this->m_header, this->m_data);
	}
	template<typename T>
	constexpr void InlineListDataBlock<T>::removeFrontWithoutDeconstruct()
	{
