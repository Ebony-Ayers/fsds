namespace fsds
{
	...
	{
		Allocator alloc;
		this->m_data = alloc.allocate(SPMCQueue<T, blockSize, Allocator>::sm_baseAllocation);
		std::allocator<SPMCQueue<T, blockSize, Allocator>::Block> allocBlock;
	...
	void SPMCQueue<T, blockSize, Allocator>::enqueueBlock(Block* block)
	{
		...
		this->m_data[this->m_lastBlock + 1] = this->m_data[this->m_lastBlock];
	...
	template<typename T, size_t blockSize, typename Allocator>
	[[nodiscard]] constexpr bool SPMCQueue<T, blockSize, Allocator>::isEmpty() noexcept
	{
		fts::GenericLockGuard lockGuard(this->m_controlBlockLock);
	...
	template<typename T, size_t blockSize, typename Allocator>
	[[nodiscard]] constexpr size_t SPMCQueue<T, blockSize, Allocator>::size() noexcept
	{
		fts::GenericLockGuard lockGuard(this->m_controlBlockLock);
	...
	template<typename T, size_t blockSize, typename Allocator>
	[[nodiscard]] constexpr size_t SPMCQueue<T, blockSize, Allocator>::capacity() noexcept
	{
		fts::GenericLockGuard lockGuard(this->m_controlBlockLock);
	...
	void SPMCQueue<T, blockSize, Allocator>::clear()
	{
		...
		this->m_size = 0;
		this->m_numBlocks = 1;
		this->m_capacity = SPMCQueue<T, blockSize, Allocator>::sm_baseAllocation;
	}
	...
	template<typename T, size_t blockSize, typename Allocator>
	constexpr void SPMCQueue<T, blockSize, Allocator>::reallocate(size_t newSize)
	{
		Allocator alloc;
	...
	template<typename T, size_t blockSize, typename Allocator>
	constexpr void SPMCQueue<T, blockSize, Allocator>::allocateNewBlock()
	{
		if(this->m_numBlocks >= this->m_capacity) [[unlikely]]
		{
			this->reallocate(this->m_capacity * 2);
	...
	template<typename T, size_t blockSize, typename Allocator>
	constexpr T& SPMCQueue<T, blockSize, Allocator>::atNoLock(const size_t& pos) const
	{
		size_t absolutePosition = pos + this->m_frontBlockOffset;
		return (this->m_data[(absolutePosition / blockSize) + this->m_firstBlock])->data[absolutePosition % blockSize];
	}
	...
	template<typename T, size_t blockSize, typename Allocator>
	constexpr bool SPMCQueue<T, blockSize, Allocator>::dataReferenceEqualityNoLock(SPMCQueue<T, blockSize, Allocator>& other)
	{
		return (this->m_data == other.m_data) && (this->m_frontBlockOffset == other.m_frontBlockOffset) && (this->m_firstBlock == other.m_firstBlock) && (this->m_lastBlock == other.m_lastBlock) && (this->m_size == other.m_size);
	}

	template<typename T, size_t blockSize, typename Allocator>
	fts::SpinLock* SPMCQueue<T, blockSize, Allocator>::getLock()
	{
