...
namespace fsds
{
	constexpr StaticString::StaticString(const CharT* str, const size_t& size)
	: m_str(str), m_size(size)
	{}
	constexpr StaticString::StaticString(const StaticString& other) noexcept
	: m_str(other.data()), m_size(other.size())
	{}

	constexpr const StaticString StaticString::operator[](size_t pos) const
	{
	...
	constexpr bool StaticString::valueEquality(const StaticString& other) const
	{
		...
		else
		{
			if(this->m_str == other.m_str)
			{
				...
				{
					StaticString character(this->m_str + codePointOffset, 1);
					auto ret = this->firstCharacterEqualityWithLength(other);
					if(ret.result == false)
					{
						return false;
					}
	...
	constexpr bool StaticString::referenceEquality(const StaticString& other) const
	{
		return ((this->m_str == other.data()) && (this->m_size == other.size()));
	}

	constexpr StaticString StaticString::substr(const size_t& pos, const size_t& len) const
	{
		size_t codePointOffset = 0;
		for(size_t i = 0; i < pos; i++)
		{
	...
	constexpr int StaticString::compare(const StaticString& other)
	{
	...
	constexpr bool StaticString::contains(const StaticString& str)
	{
		return find(str) != StaticString::npos;
	}

	constexpr size_t StaticString::find(const StaticString& str) const
	{
	...
	constexpr StaticStringItterator StaticString::findItterator(const StaticString& str) const
	{
		if(str.size() > this->m_size) [[unlikely]]
		{
	...
	constexpr size_t StaticString::findFirstCharacter(const StaticString& str) const
	{
		size_t codePointOffset = 0;
		for(size_t i = 0; i < this->m_size; i++)
		{
	...
	constexpr StaticStringItterator StaticString::findFirstCharacterItterator(const StaticString& str) const
	{
		size_t codePointOffset = 0;
		for(size_t i = 0; i < this->m_size; i++)
		{
	...
	constexpr StaticString::firstCharacterEqualityWithLengthReturnType StaticString::firstCharacterEqualityWithLength(const StaticString& str) const
	{
		size_t numCodePoints = this->numCodePointsInFirstCharacter();
		//if the number of code points are no equal then the two characters cannot be the same and functions as a safety check in the case of one string being a single character of lesser length thus overflowing the smaller string
		if(numCodePoints != str.numCodePointsInFirstCharacter())
	...
	constexpr StaticStringItterator::StaticStringItterator(const StaticString* sStr)
	: m_str(sStr), m_pos(0), m_chrIndex(0)
	{}
	constexpr StaticStringItterator::StaticStringItterator(const StaticString* sStr, const size_t& pos, const size_t& codePoint)
	: m_str(sStr), m_pos(pos), m_chrIndex(codePoint)
	{}
	constexpr StaticStringItterator::StaticStringItterator(const StaticString* sStr, const bool& isNPos)
	: m_str(sStr), m_pos(0), m_chrIndex(0)
	{
		if(isNPos)
		{
	...
	constexpr StaticString StaticStringItterator::get() const
	{
		return StaticString(m_str->data() + this->m_pos, 1);
	}

	constexpr bool StaticStringItterator::next()
	{
	...
	constexpr bool StaticStringItterator::previous()
	{
		StaticString::CharT codePoint = this->m_str->data()[this->m_pos - 1];
