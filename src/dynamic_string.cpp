...
namespace fsds
{
	...
	bool DynamicString::referenceEquality(const DynamicString& other) const
	{
	...
	bool DynamicString::referenceEquality(const StaticString& other) const
	{
		return ((this->m_str == other.data()) && (this->m_size == other.size()));
	}
	...
	int DynamicString::compare(const DynamicString& other)
	{
	...
	bool DynamicString::contains(const DynamicString& str)
	{
	...
	bool DynamicString::firstCharacterEquality(const DynamicString& str) const
	{
	...
	DynamicString::firstCharacterEqualityWithLengthReturnType DynamicString::firstCharacterEqualityWithLength(const DynamicString& str) const
	{
		size_t numCodePoints = this->numCodePointsInFirstCharacter();
		//if the number of code points are no equal then the two characters cannot be the same and functions as a safety check in the case of one string being a single character of lesser length thus overflowing the smaller string
		if(numCodePoints != str.numCodePointsInFirstCharacter())
	...
	size_t DynamicString::numCodePointsInFirstCharacter(size_t codePointOffset) const
	{
	...
	void DynamicString::reallocate(size_t newSize)
	{
		std::allocator<DynamicString::CharT> alloc;
